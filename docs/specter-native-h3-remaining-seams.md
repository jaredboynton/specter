...
| Area | Closed state |
|---|---|
| Same-fixture H3 GET superiority | The GET-only ledger repeat gate passed twice consecutively at `ba356d7`: Specter worst rep ahead of every comparator per-metric best rep on all six metrics (TTFB p50/p95, ledger throughput, ledger tail p50/p95). |
| RFC 9220 steadystate tunnel tail | Specter holds the lower p95 round-trip tail on echo, close/FIN, and mixed under symmetric warm-vs-warm measurement; the prior echo/close p95 losses were warm-vs-cold asymmetry plus the since-fixed inline DPLPMTUD probe spike (`5e0d429`). |
...
| RFC9220 comparator rows | Specter echo/close/mixed rows and low-level `quiche`/`tokio-quiche` echo/close/mixed rows are persisted under the fair steadystate harness. |
...
| `docs/benchmarks/native-h3-vs-rust-clients/2026-06-09-pmtu-probe-tunnel-defer/` | Fair warm-vs-warm steadystate RFC 9220 tunnel result. | Specter lower p95 tail on echo, close, mixed (8 reps, n=100); strict full-suite gate still fails on echo p50/throughput parity. |
...
env -i HOME="$HOME" USER="$USER" PATH="/usr/local/bin:/usr/bin:/bin:$HOME/.cargo/bin" \
  ...
  SPECTER_NATIVE_H3_DIRECT_GET_IO_EPOCH=1 ARCHIVE_NAME=<fresh-dir> ARCHIVE_SLUG=<slug> \
