...
fn competitor_specs() -> Vec<CompetitorSpec> {
    vec![
        ...
        CompetitorSpec {
            id: "quiche_direct_rfc9220_tunnel_close",
            crate_name: "quiche",
            version: "0.29.0",
            role: "h3_tunnel_comparator",
            required_for_superiority: false,
            invocation_notes:
                "Low-level quiche RFC 9220 tunnel comparator for client DATA+FIN and server FIN timing.",
        },
        CompetitorSpec {
            id: "tokio_quiche_rfc9220_tunnel_close",
            crate_name: "tokio-quiche",
            version: "0.19.0",
            role: "h3_tunnel_comparator",
            required_for_superiority: false,
            invocation_notes:
                "tokio-quiche RFC 9220 tunnel comparator for client DATA+FIN and server FIN timing.",
        },
        CompetitorSpec {
            id: "quiche_direct_rfc9220_tunnel_mixed",
            crate_name: "quiche",
            version: "0.29.0",
            role: "h3_tunnel_comparator",
            required_for_superiority: false,
            invocation_notes:
                "Low-level quiche RFC 9220 slow-consumer tunnel plus concurrent H3 streaming comparator.",
        },
        CompetitorSpec {
            id: "tokio_quiche_rfc9220_tunnel_mixed",
            crate_name: "tokio-quiche",
            version: "0.19.0",
            role: "h3_tunnel_comparator",
            required_for_superiority: false,
            invocation_notes:
                "tokio-quiche RFC 9220 slow-consumer tunnel plus concurrent H3 streaming comparator.",
        },
        CompetitorSpec {
...
fn quiche_direct_rfc9220_tunnel_mixed_row_from_samples(samples: &[AdapterSample]) -> BenchmarkRow {
    adapter_row_from_samples(
        "quiche_direct_rfc9220_tunnel_mixed",
        "quiche_direct_rfc9220_tunnel_mixed_adapter",
        samples,
    )
}

fn tokio_quiche_rfc9220_tunnel_mixed_row_from_samples(samples: &[AdapterSample]) -> BenchmarkRow {
    adapter_row_from_samples(
        "tokio_quiche_rfc9220_tunnel_mixed",
        "tokio_quiche_rfc9220_tunnel_mixed_adapter",
        samples,
    )
}
...
fn row_context(competitor_id: &str) -> Option<RowContext> {
    match competitor_id {
        ...
        "quiche_direct_rfc9220_tunnel_close" | "tokio_quiche_rfc9220_tunnel_close" => {
            Some(RowContext {
                protocol: "h3_rfc9220",
                workload: "websocket_over_h3_raw_tunnel_close_fin",
                default_payload_bytes: Some(LOCAL_FIXTURE_TUNNEL_PAYLOAD_SIZE),
                notes: Some("RFC9220 comparator adapter measuring client DATA+FIN through echoed DATA and server FIN delivery."),
            })
        }
        "quiche_direct_rfc9220_tunnel_mixed" | "tokio_quiche_rfc9220_tunnel_mixed" => {
            Some(RowContext {
                protocol: "h3_rfc9220",
                workload: "slow_consumer_tunnel_plus_http3_streaming",
                default_payload_bytes: Some(
...
fn local_native_fixture_measurement_plan() -> Vec<&'static str> {
    ...
    plan.push("tokio_quiche_rfc9220_tunnel_close");
    plan.push("quiche_direct_rfc9220_tunnel_mixed");
    plan.push("tokio_quiche_rfc9220_tunnel_mixed");
    plan
}
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(url) = option_value(&args, "--measure-quiche-direct-rfc9220-tunnel-close-url") {
        measured_competitor_rows.push(
            measure_quiche_direct_rfc9220_tunnel_close(
                &url,
                option_usize(&args, "--warmups", 3)?,
                option_usize(&args, "--samples", 30)?,
            )
    ...
    if let Some(url) = option_value(&args, "--measure-tokio-quiche-rfc9220-tunnel-close-url") {
        measured_competitor_rows.push(
            measure_tokio_quiche_rfc9220_tunnel_close(
                &url,
                option_usize(&args, "--warmups", 3)?,
                option_usize(&args, "--samples", 30)?,
            )
        ...
        option_value(&args, "--measure-quiche-direct-rfc9220-tunnel-mixed-url")
    {
        let stream_url = option_value(
            &args,
            "--measure-quiche-direct-rfc9220-tunnel-mixed-stream-url",
        )
        .ok_or_else(|| {
            anyhow::anyhow!(
                "--measure-quiche-direct-rfc9220-tunnel-mixed-url requires --measure-quiche-direct-rfc9220-tunnel-mixed-stream-url"
            )
        })?;
        measured_competitor_rows.push(
            measure_quiche_direct_rfc9220_tunnel_mixed(
                &stream_url,
                &tunnel_url,
                option_usize(&args, "--warmups", 3)?,
                option_usize(&args, "--samples", 30)?,
    ...
    if let Some(tunnel_url) = option_value(&args, "--measure-tokio-quiche-rfc9220-tunnel-mixed-url")
    {
        let stream_url = option_value(
            &args,
            "--measure-tokio-quiche-rfc9220-tunnel-mixed-stream-url",
        )
        .ok_or_else(|| {
            anyhow::anyhow!(
                "--measure-tokio-quiche-rfc9220-tunnel-mixed-url requires --measure-tokio-quiche-rfc9220-tunnel-mixed-stream-url"
            )
        })?;
        measured_competitor_rows.push(
            measure_tokio_quiche_rfc9220_tunnel_mixed(
                &stream_url,
                &tunnel_url,
                option_usize(&args, "--warmups", 3)?,
                option_usize(&args, "--samples", 30)?,
...
async fn measure_local_native_fixture(
    ...
    for client in local_native_fixture_measurement_plan_for(selected_client)? {
        ...
        let mut row = match client {
            ...
            "quiche_direct_rfc9220_tunnel_close" => {
                let tunnel_url = fixture.tunnel_url();
                measure_quiche_direct_rfc9220_tunnel_close(&tunnel_url, warmups, samples).await
            }
            "tokio_quiche_rfc9220_tunnel_close" => {
                let tunnel_url = fixture.tunnel_url();
                measure_tokio_quiche_rfc9220_tunnel_close(&tunnel_url, warmups, samples).await
            }
            "quiche_direct_rfc9220_tunnel_mixed" => {
                let tunnel_url = fixture.tunnel_url();
                measure_quiche_direct_rfc9220_tunnel_mixed(
    ...
    async fn run(mut self) {
        loop {
            ...
            tokio::select! {
                ...
                packet = self.rx.recv() => {
                    let Some((packet, peer)) = packet else { break };
                    if let Err(error) = self.process_datagram(&packet, peer).await {
...
fn measure_quiche_direct_rfc9220_tunnel_once_with_payload(
    ...
    while Instant::now() < deadline {
        loop {
            match socket.recv_from(&mut recv_buf) {
                Ok((len, from)) => {
                    ...
                    if let Err(err) = conn.recv(&mut recv_buf[..len], recv_info) {
                        if err != quiche::Error::Done {
                            return Err(anyhow::anyhow!("quiche RFC 9220 recv failed: {err:?}"));
                        }
                    }
                }
                Err(err) if err.kind() == io::ErrorKind::WouldBlock => break,
        ...
        if conn.is_closed() {
            anyhow::bail!("quiche RFC 9220 connection closed before tunnel echo completed");
        }
        ...
        if let Some(http3) = h3_conn.as_mut() {
            if !req_sent {
                ...
                stream_id = Some(opened_stream_id);
                req_sent = true;
            }
            ...
            loop {
                match http3.poll(&mut conn) {
                    ...
                    Ok((event_stream_id, quiche::h3::Event::Data)) => loop {
                        match http3.recv_body(&mut conn, event_stream_id, &mut recv_buf) {
                            Ok(read) => echoed.extend_from_slice(&recv_buf[..read]),
                            Err(quiche::h3::Error::Done) => break,
                            Err(err) => {
                                return Err(anyhow::anyhow!(
                                    "quiche RFC 9220 h3 body failed: {err:?}"
                                ));
                            }
                        }
                    },
                    Ok((event_stream_id, quiche::h3::Event::Finished)) => {
                        if Some(event_stream_id) != stream_id {
                            anyhow::bail!(
                                "quiche RFC 9220 finished unexpected stream {event_stream_id}"
                            );
                        }
                        if echoed.as_slice() != payload.as_ref() {
                            anyhow::bail!(
                                "quiche RFC 9220 echo mismatch: expected {} bytes, got {} bytes",
                                payload.len(),
                                echoed.len()
                            );
                        }
                        let total_ns = start.elapsed().as_nanos() as f64;
                        return Ok(AdapterSample::new(total_ns, total_ns, echoed.len() as u64));
                    }
                    Ok((_, quiche::h3::Event::Reset(error_code))) => {
                        anyhow::bail!("quiche RFC 9220 h3 stream reset: {error_code}");
                    }
                    Ok((_, quiche::h3::Event::PriorityUpdate | quiche::h3::Event::GoAway)) => {}
                    Err(quiche::h3::Error::Done) => break,
                    Err(err) => {
...
async fn read_tokio_quiche_rfc9220_tunnel_echo(
    ...
    let mut echoed = Vec::new();
    if headers.read_fin {
        anyhow::bail!("tokio_quiche RFC 9220 response ended before tunnel echo");
    }
    ...
    loop {
        ...
        match frame {
            tokio_quiche::http3::driver::InboundFrame::Body(chunk, fin) => {
                ...
                if fin {
                    if echoed.as_slice() != expected_payload.as_ref() {
                        anyhow::bail!(
                            "tokio_quiche RFC 9220 echo mismatch: expected {} bytes, got {} bytes",
...
fn rfc9220_request_authority(url: &url::Url) -> anyhow::Result<String> {
    let host = url
        .host_str()
...
fn quiche_rfc9220_tunnel_headers(url: &url::Url) -> anyhow::Result<Vec<quiche::h3::Header>> {
    let authority = rfc9220_request_authority(url)?;
    let path = rfc9220_request_path(url);
    Ok(vec![
        ...
        quiche::h3::Header::new(b"user-agent", b"quiche-direct-rfc9220"),
    ])
}

fn tokio_quiche_rfc9220_tunnel_headers(
    url: &url::Url,
) -> anyhow::Result<Vec<tokio_quiche::quiche::h3::Header>> {
    let authority = rfc9220_request_authority(url)?;
    let path = rfc9220_request_path(url);
    Ok(vec![
        tokio_quiche::quiche::h3::Header::new(b":method", b"CONNECT"),
        tokio_quiche::quiche::h3::Header::new(b":protocol", b"websocket"),
        ...
        tokio_quiche::quiche::h3::Header::new(b"user-agent", b"tokio-quiche-rfc9220"),
    ])
}
...
mod tests {
    ...
    fn local_native_fixture_plan_includes_feature_enabled_clients() {
        ...
        expected.push("quiche_direct_rfc9220_tunnel_mixed");
        expected.push("tokio_quiche_rfc9220_tunnel_mixed");
    ...
    #[test]
    fn quiche_direct_rfc9220_tunnel_mixed_adapter_row_uses_measured_samples() {
        let samples = vec![
            super::AdapterSample::new(80.0, 800.0, 8_000),
            super::AdapterSample::new(10.0, 100.0, 1_000),
            super::AdapterSample::new(50.0, 500.0, 5_000),
        ];

        let row = super::quiche_direct_rfc9220_tunnel_mixed_row_from_samples(&samples);
        ...
        assert_eq!(
            row.workload.as_deref(),
            Some("slow_consumer_tunnel_plus_http3_streaming")
        );
    }
    ...
    #[test]
    fn tokio_quiche_rfc9220_tunnel_mixed_adapter_row_uses_measured_samples() {
        let samples = vec![
            super::AdapterSample::new(90.0, 900.0, 9_000),
            super::AdapterSample::new(10.0, 100.0, 1_000),
            super::AdapterSample::new(60.0, 600.0, 6_000),
        ];

        let row = super::tokio_quiche_rfc9220_tunnel_mixed_row_from_samples(&samples);
    ...
    #[tokio::test]
    async fn quiche_direct_local_fixture_measures_rfc9220_tunnel_slow_consumer_mixed_workload() {
        let fixture = super::LocalNativeH3Fixture::start("quiche_direct_rfc9220_tunnel_mixed")
            .await
            .unwrap();

        let row = super::measure_quiche_direct_rfc9220_tunnel_mixed(
            fixture.stream_url(),
            &fixture.tunnel_url(),
            0,
            1,
    ...
    #[tokio::test]
    async fn tokio_quiche_local_fixture_measures_rfc9220_tunnel_slow_consumer_mixed_workload() {
        let fixture = super::LocalNativeH3Fixture::start("tokio_quiche_rfc9220_tunnel_mixed")
            .await
            .unwrap();

        let row = super::measure_tokio_quiche_rfc9220_tunnel_mixed(
            fixture.stream_url(),
            &fixture.tunnel_url(),
            0,
            1,
