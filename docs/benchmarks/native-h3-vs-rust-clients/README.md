...
- It was produced from n=100 per-client fixture runs (`--warmups 5 --samples 100`) for `specter_native`, `quiche_direct`, `tokio_quiche`, `h3_quinn`, `reqwest_h3`, all nine required RFC9220 tunnel rows above, and measured `quinn_transport` / `s2n_quic_transport` rows, then merged through the benchmark import-precedence path.
...
- Every measured H3 HTTP and RFC9220 gate row in the suite artifact carries `sample_count = 100`; transport-only baseline rows are measured non-gate rows and do not carry the H3/RFC9220 sample-count contract.
