...
this measurable; before it, Warpsock DNF'd every netem run -- see
...
`--warmups 0 --samples 100`, 3 reps, publishable Warpsock GET profile (IDLE_GET=1,
...
`WARPSOCK_BENCH_RUN_PROVENANCE`, and fabricating that stamp for a diagnostic would
be a benchmark-truth violation; all clients run identical conditions so the
comparison is fair.

## Question

Is Warpsock's RTT~0 GET ledger-paced-tail loss (~23us vs tokio ~15us at the canonical
...
|---------------|--------|--------|--------|--------|--------|--------|
| warpsock_native| 17.9   | 20.5   | 15.7   | 55.1   | 49.6   | 46.5   |
...
p50 TTFB (us): warpsock 433.2-435.9 (best of five every rep, tied with quiche_direct),
h3_quinn 441-445, tokio 445-446, reqwest 452-453. Throughput: parity (15.9-16.4
MB/s all clients).

## Conclusions

1. The artifact thesis is refuted. Warpsock's tail deficit vs tokio_quiche is a
...
2. Warpsock wins TTFB at real RTT and holds throughput parity; the tail is the one
...
3. quiche_direct's tail collapses at RTT (3-5x worse than at RTT~0, last place by
   ...
   is consistent with the architecture-wave finding that the remaining Warpsock
   deficit is loop-structural (single pinned timer + drain-to-quiescence in quinn
   and tokio-quiche vs Warpsock's per-iteration select! arms), and that closing it
   is an architectural change to the receive loop rather than another micro-lever.
4. Rep noise at RTT is large enough that Warpsock's best p95 (46.5) beats tokio's
